...
//			if (rank == 0)
//				total_time_buffer = (float*)malloc(count*sizeof(float)*file->comm->simulation_nprocs);
//
...
//				total_time_buffer = (float*)malloc(count*10*sizeof(float)*file->comm->simulation_nprocs);
//
...
//			if (rank == 0)
//				total_size_buffer = (long long int*)malloc(count*sizeof(long long int)*file->comm->simulation_nprocs);
//
