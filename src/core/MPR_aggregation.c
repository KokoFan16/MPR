...
//	MPI_Allgather(&patch_count, 1, MPI_INT, patch_count_array, 1, MPI_INT, comm);
	...
	int local_patch_size_id[max_pcount * 2];
	memset(local_patch_size_id, -1, max_pcount * 2 * sizeof(int));
	for (int i = 0; i < patch_count; i++)
	{
		local_patch_size_id[i * 2] = file->variable[svi]->local_patch->patch[i]->patch_buffer_size;
		local_patch_size_id[i * 2 + 1] = file->variable[svi]->local_patch->patch[i]->global_id;
	}

	int* patch_size_id = malloc(max_pcount * proc_num * 2 * sizeof(int));
	MPI_Allgather(local_patch_size_id, max_pcount * 2, MPI_INT, patch_size_id, max_pcount * 2, MPI_INT, comm);
	...
	{
		int id = patch_size_id[i*2 + 1];
		if (id > -1)
			patch_sizes[id] = patch_size_id[i*2];
	}
	free(patch_size_id);
		...
		while (p < patch_count_power2)
		{
			if (agg_sizes[a] < average_file_size)
			{
				if (patch_ids_zorder[p] > -1)
					patch_assign_array[patch_ids_zorder[p]] = a;
