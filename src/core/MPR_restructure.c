...
MPR_return_code MPR_restructure_perform(MPR_file file, int start_var_index, int end_var_index)
{
	...
	int* global_box = file->mpr->global_box;  /* The size of global dataset */
	int* patch_box = file->mpr->patch_box; /* The size of patch */
	int patch_size = patch_box[0] * patch_box[1] * patch_box[2];
	/***************************************************************************/
	...
	int local_box_end[MPR_MAX_DIMENSIONS];
	int cur_local_end[MPR_MAX_DIMENSIONS];
	for (int d = 0; d < MPR_MAX_DIMENSIONS; d++)
	{
		cur_local_offset[d] = (file->mpr->local_offset[d]/patch_box[d]);
		local_box_end[d] = file->mpr->local_offset[d] + file->mpr->local_box[d];
		cur_local_end[d] = ceil(local_box_end[d]/(float)patch_box[d]);
	}
	...
	/***************************** Find single and shared patches *******************************/
	int process_dimensional_counts[MPR_MAX_DIMENSIONS];
	for (int d = 0; d < MPR_MAX_DIMENSIONS; d++)
		process_dimensional_counts[d] = ceil((float)global_box[d]/file->mpr->local_box[d]);

	int patch_offsets[total_patch_num][MPR_MAX_DIMENSIONS];
	int patches_are_single[total_patch_num];
	memset(patches_are_single, 0, total_patch_num * sizeof(int));
	int patches_are_boundary[total_patch_num];
	memset(patches_are_boundary, 0, total_patch_num * sizeof(int));

	int shared_pcount = 0;
	int shared_patches[total_patch_num];
	...
	for (int k = 0; k < global_box[2]; k += patch_box[2])
	{
		for (int j = 0; j < global_box[1]; j += patch_box[1])
		{
			for (int i = 0; i < global_box[0]; i += patch_box[0])
			{
				...
				int dif_sum = 0;
				int offset_div[MPR_MAX_DIMENSIONS];
				for (int d = 0; d < MPR_MAX_DIMENSIONS; d++)
				{
					if (end[d] > global_box[d])
					{
						patches_are_boundary[global_id] = 1;
						end[d] = global_box[d];
					}
					offset_div[d] = start[d] / file->mpr->local_box[d];
					dif_sum += offset_div[d] - (end[d] - 1) / file->mpr->local_box[d];
				}

				if (dif_sum == 0)
				{
					int process_id = offset_div[2] * process_dimensional_counts[1] * process_dimensional_counts[0] +
							offset_div[1] * process_dimensional_counts[0] + offset_div[0];
					cur_assign_patch_num[process_id] += 1;
					patches_are_single[global_id] = 1;
					patch_assignment[global_id] = process_id;
					shared_rank_count[global_id] = 1;
					shared_patch_ranks[global_id][0] = process_id;
					if (rank == process_id)
						local_assigned_patches[local_assigned_count++] = global_id;
				}
				else
					shared_patches[shared_pcount++] = global_id;
	...
	if (shared_pcount > 0)
	{
		/***************************** Find processes of shared patches *******************************/
		for (int i = 0; i < shared_pcount; i++)
		{
			int patch_id = shared_patches[i];
			int end[MPR_MAX_DIMENSIONS] = {patch_offsets[patch_id][0] + patch_box[0],
					patch_offsets[patch_id][1] + patch_box[1], patch_offsets[patch_id][2] + patch_box[2]};
			...
			for (int d = 0; d < MPR_MAX_DIMENSIONS; d++)
			{
				...
				start_dimensions[d] = patch_offsets[patch_id][d] / file->mpr->local_box[d];
				end_dimensions[d] = (end[d] - 1) / file->mpr->local_box[d];
			}
			...
			for (int z = start_dimensions[2]; z < end_dimensions[2] + 1; z++)
				for (int y = start_dimensions[1]; y < end_dimensions[1] + 1; y++)
					for (int x = start_dimensions[0]; x < end_dimensions[0] + 1; x++)
					{
						...
						shared_patch_ranks[patch_id][shared_rank_count[patch_id]++] = process_id;
					}
		}

		/******************************* Assigned shared patches *******************************/
		for (int i = 0; i < shared_pcount; i++)
		{
			int flag = 0; /* If the patch has been assigned to any process */
			int assigned_rank = 0;
			int patch_id = shared_patches[i];
			for (int p = 0; p < shared_rank_count[patch_id]; p++)
			{
			...
			cur_assign_patch_num[assigned_rank]++;
			patch_assignment[patch_id] = assigned_rank;

			if (rank == assigned_rank)
				local_assigned_patches[local_assigned_count++] = shared_patches[i];
		}
	}

	/* Assign Error happens */
	if (local_assigned_count != local_patch_num)
	{
	...
	for (int v = start_var_index; v < end_var_index; v++)
	{
		...
		for (int i = 0; i < local_patch_num; i++)
		{
			...
			if (patches_are_single[patch_id] == 1 && patches_are_boundary[patch_id] == 0)
			{
				int process_id = shared_patch_ranks[patch_id][0];
				MPI_Irecv(local_patch->patch[i]->buffer, patch_size * bytes, MPI_BYTE, process_id, patch_id, comm, &req[req_i]);
				req_i++;
			}
			else
			{
				...
				for (int j = 0; j < shared_processes_count; j++)
				{
					...
					int z = process_id / (process_dimensional_counts[1] * process_dimensional_counts[0]);
					int remain = process_id -  z * process_dimensional_counts[1] * process_dimensional_counts[0];
					int y = remain / process_dimensional_counts[0];
					int x = remain % process_dimensional_counts[0];

					int local_offset[MPR_MAX_DIMENSIONS] = {x * file->mpr->local_box[0], y * file->mpr->local_box[1], z * file->mpr->local_box[2]};
					...
					int array_subsize[MPR_MAX_DIMENSIONS] = {physical_size[0] * bytes,
							physical_size[1], physical_size[2]};
					int subarray_offset[MPR_MAX_DIMENSIONS] = {patch_share_offset[0] * bytes,
							patch_share_offset[1], patch_share_offset[2]};
		...
		int send_array[MPR_MAX_DIMENSIONS] = {file->mpr->local_box[0]*bytes, file->mpr->local_box[1], file->mpr->local_box[2]};
		for (int i = 0; i < local_own_patch_count; i++)
		{
			...
			for (int d = 0; d < MPR_MAX_DIMENSIONS; d++)
			{
				...
				if (patch_end[d] > local_box_end[d])
					physical_size[d] = local_box_end[d] - patch_offsets[patch_id][d];
				if (patch_offsets[patch_id][d] < file->mpr->local_offset[d])
				{
			...
			MPI_Type_create_subarray(MPR_MAX_DIMENSIONS, send_array, physical_size, send_offset, MPI_ORDER_FORTRAN, MPI_BYTE, &send_type);
